...
draw :: proc() {
    for &s in snakes {
        ...
        for i in 0..<len(s.chain.joints) {
            thicknesses[i] = joints_radius(i) * s.body
        }
        draw_smooth_snake(s.chain.joints[:], thicknesses, s.color1, s.color2)
        delete(thicknesses)
...
draw2 :: proc() {
    for &s in snakes {
        ...
        for i in 1..<len(c.joints) {
            rl.DrawCircleV(c.joints[i], joints_radius(i) * s.body, color)
        }

        rotation: f32 = 0.0
        for i in 0..<len(c.joints) {
            size := joints_radius(i) * s.body * 2.0
            rotation = c.angles[i]  * 180 / f32(math.PI)
            rec := rl.Rectangle{
                x =  c.joints[i].x,
                y =  c.joints[i].y,
                width = size,
            ...
            rl.DrawRectanglePro(rec, origin, rotation, s.color1)
        }
        ...
        for i in 0..<1 {
            ...
            rotation = c.angles[i+1]  * 180 / f32(math.PI) + 45
            head_rec = rl.Rectangle{
                x =  c.joints[i].x,
                y =  c.joints[i].y,
                width = size,
            ...
            origin := rl.Vector2{size / 2.0, size / 2.0 }
            rl.DrawRectanglePro(head_rec, origin, rotation, s.color1)
        }
        ...
        // spine
        color = rl.Color{255, 255, 255, 153}
        for i in 1..<len(c.joints) {
            rl.DrawLineEx(c.joints[i-1], c.joints[i], SPINE_THICKNESS, color)
            rl.DrawCircleV(c.joints[i], joints_radius(i) * s.body * 0.25, color)
        }

        //b brain
        rl.DrawCircleV(c.joints[0], joints_radius(0) * s.body *0.6, rl.Color{255, 255, 255, 204})
        rl.DrawCircleV(c.joints[0], joints_radius(0) * s.body *0.5, rl.Color{255, 0, 0, 153})
        ...
        for i in 0..<len(corners) {
            if i % 2 == 0 {
                ...
                // nose
                rl.DrawCircleV(corners[i], r * 0.15, s.color1)
            }
        }
    }
}
...
init_snakes:: proc() {
    for i in 0..<SNAKE_COUNT {
        ...
        snakes[i].id = i
        snakes[i].vel.x = math.cos(angle) * speed
        snakes[i].vel.y = math.sin(angle) * speed
        snakes[i].body = body
        snakes[i].color1 = color
        snakes[i].color2 = get_complementary_color(color)

        chain := new_chain(origin, joints, links, math.PI / 4.0)
        snakes[i].chain = chain
